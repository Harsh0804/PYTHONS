#R.K.Computer Classes
#Lord MAHADEV Sketch 
 


#import all the needed file first
import turtle as tu
import cv2
...
class sketch_from_svg:
    #create the function as required
    ...
    def hex_to_rgb(self,string):
        #defining the string length
        strlen = len(string)
        #here we uses the if else ladder and nested if to define the condition
    ...
    #creating a function for image source
    def load_svg(self):
        print('loading data')
        ...
        return res
    #define function for all pen and moving module
    def move_to(self,x, y):
        self.pen.up()
        self.pen.goto(x,y)
        self.pen.down()
        #declaring function for drawing
    def draw(self,retain=True):
        ...
        if retain == True:
            tu.done()
# Assing the image path to the pen predefined variable
pen= sketch_from_svg('mahadev.svg',scale=70)  
pen.draw()